...
#from model import compose2

def conv(in_planes, out_planes, kernel_size=3, use_nonlocal = False):
    if use_nonlocal == False:
        return nn.Sequential(
            nn.Conv2d(in_planes, out_planes, kernel_size=kernel_size, padding=(kernel_size-1)//2, stride=2),
            nn.ReLU(inplace=False) #was LeakyRelu for the best result
        )
    elif use_nonlocal == True:
        return nn.Sequential(
            NONLocalBlock2D(in_channels=in_planes),
            nn.Conv2d(in_planes, out_planes, kernel_size=kernel_size, padding=(kernel_size-1)//2, stride=2),
            nn.ReLU(inplace=False) #modified
        )

def upconv(in_planes, out_planes,use_nonlocal = False):
    if use_nonlocal == False:
        return nn.Sequential(
...
class PoseExpNet(nn.Module):
    ...
    def __init__(self, nb_ref_imgs=2,p_enable = False, output_exp=False,use_nonlocal = False):
        ...
        self.conv3 = conv(conv_planes[1], conv_planes[2],use_nonlocal = use_nonlocal)
        self.conv4 = conv(conv_planes[2], conv_planes[3])
        self.conv5 = conv(conv_planes[3], conv_planes[4],use_nonlocal = use_nonlocal)
        self.conv6 = conv(conv_planes[4], conv_planes[5])
        self.conv7 = conv(conv_planes[5], conv_planes[6])
        if self.nb_ref_imgs == 0:
            self.nb_ref_imgs = 2
        self.pose_pred = nn.Conv2d(conv_planes[6], 6*self.nb_ref_imgs, kernel_size=1, padding=0)

        self.final_pose = nn.Sequential(
        nn.Linear(12,12), #12
        nn.ReLU(),
        nn.Linear(12,6)
        )
    ...
    def init_weights(self):
        for m in self.modules():
            if isinstance(m, nn.Conv2d) or isinstance(m, nn.ConvTranspose2d):
                xavier_uniform_(m.weight.data) #was xavier_uniform
                if m.bias is not None:
                    constant_(m.bias,0)

    def forward(self, target_image, ref_imgs = None,p = None):
        ...
        if self.p_enable == True:
            pose = self.final_pose(torch.cat([pose,p],2)) 
            #pose = self.final_pose(pose + p)
        if self.output_exp:
